(() => {
  ...
  const builderDefaults = {
    geo: {
      place: 'Les Sables-d’Olonne', area: 'west', country: 'France', nearby: 'the Atlantic Ocean', feature: 'its long beaches and harbour'
    },
    family: {
    ...
    story: {
      when: 'Last month', destination: 'Iceland', companions: 'my family', sights: 'volcanoes, waterfalls and icebergs', weather: 'sunny but cool', feeling: 'amazed by the landscape'
    }
  };
  ...
  function builderText(type, level, values) {
    ...
    if (type === 'story') {
      if (level === 'a2') return `${values.when}, I went to ${values.destination} with ${values.companions}. We saw ${values.sights}. The weather was ${values.weather}. I was ${values.feeling}. It was a beautiful trip.`;
      if (level === 'a2plus') return `${values.when}, I travelled to ${values.destination} with ${values.companions}. First, we explored the area and saw ${values.sights}. Luckily, the weather was ${values.weather}. I felt ${values.feeling}, and I really enjoyed sharing this experience with my family.`;
      return `One of my most memorable experiences was a trip to ${values.destination} with ${values.companions}. During our stay, we discovered ${values.sights}, which made the landscape feel truly exceptional. Although the weather was ${values.weather}, I was ${values.feeling}. Above all, the trip was special because it gave us the opportunity to create wonderful memories together.`;
    }
    return '';
  }
  ...
  noteFields.forEach(id => {
    ...
    const stored = safeStorage.get(`mc_lesson1_${id}`);
    if (stored !== null) field.value = stored;
  });
  $('#saveNotes')?.addEventListener('click', () => {
    const saved = noteFields.map(id => safeStorage.set(`mc_lesson1_${id}`, document.getElementById(id).value)).every(Boolean);
  ...
  $('#downloadNotes')?.addEventListener('click', () => {
    const content = [
      'MARIE-CHRISTINE — LESSON 1 NOTES',
      'Your English Journey Passport',
      '',
    ...
    downloadText(content, 'Marie-Christine-Lesson-1-Notes.txt');
  });
  ...
  function evaluationText() {
    return [
      'MARIE-CHRISTINE — LESSON 1 FEEDBACK',
      'Your English Journey Passport',
      '',
  ...
  $('#downloadEvaluation')?.addEventListener('click', () => {
    downloadText(evaluationText(), 'Marie-Christine-Lesson-1-Feedback.txt');
