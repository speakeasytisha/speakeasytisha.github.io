(function () {
  ...
  function buildInfoRecommendation(interest) {
    const key = (interest || "").toLowerCase();

    // Museums / history
    if (key.includes("museum") || key.includes("history")) {
      return "You could start with the British Museum, then head to the South Bank for a walk along the Thames and views of Tower Bridge.";
    }

    // Parks / nature
    if (key.includes("park") || key.includes("nature") || key.includes("green")) {
      return "You could visit Hyde Park and Kensington Gardens, then walk through St James’s Park towards Buckingham Palace.";
    }

    // Shopping / lively streets
    if (key.includes("shopping") || key.includes("busy") || key.includes("street")) {
      return "For busy streets and shopping, try Oxford Street and Regent Street, then Covent Garden and Soho. If you like markets, Camden Market is fun.";
    }
  ...
  if (infoForm && infoOutput) {
    infoForm.addEventListener("submit", (e) => {
      ...
      const extra = infoForm.extra.value || "It’s my first time in London.";

      function buildVisitorRequest(interest, day) {
        const key = (interest || "").toLowerCase();
        const when = day || "today";

        if (key.includes("shopping") || key.includes("busy") || key.includes("street")) {
          return "I’d like to do some shopping and explore lively streets " + when + ".";
        }
        if (key.includes("park") || key.includes("nature") || key.includes("green")) {
          return "I’d like to spend some time in parks and green spaces " + when + ".";
        }
        if (key.includes("museum") || key.includes("history")) {
          return "I’d like to visit a museum and learn a bit of history " + when + ".";
        }
        return "I’d like some ideas for things to do " + when + ".";
      }

      const visitorRequest = buildVisitorRequest(interest, day);

      const recommendation = buildInfoRecommendation(interest);

      const text =
        "Tourist Information Officer: Good morning. How can I help you?\n" +
        "Visitor: Hello. My name is " + name + ". I’m from " + fromCountry + ", and I live in " + liveIn + ".\n" +
        "Visitor: I’m staying " + stay + ". " + extra + "\n" +
        "Visitor: " + visitorRequest + " What do you recommend?\n" +
        "Tourist Information Officer: " + recommendation + "\n" +
        "Visitor: That sounds great. Thank you very much!";
...
/* ==============================
  ...
  if (travelForm && travelOutput) {
    travelForm.addEventListener("submit", (e) => {
      ...
      const reason = travelForm.reason.value || "because it is the fastest option";
      ...
      const text =
        ...
        name + ": It takes " + time + ". I chose this option " + reason + ". I’m travelling " + withWho + ".\n" +
        "Ticket agent: Great. Here is your ticket. Have a good journey!\n" +
        name + ": Thank you!\n\n" +
        "Narrator: " + name + " is on the way to London. They are looking out of the window and practising their English. " +
        "They feel a little nervous, but they’re excited to visit London.";
...
/* ==============================
  ...
  ferryForm.addEventListener("submit", (e) => {
    ...
    // Passenger answer (matches your dropdown values)
    const passengerAnswer =
      passengerType === "on foot"
        ? "I’m travelling on foot."
        : "I’m travelling " + passengerType + ".";

    // Cabin answer (matches your dropdown values)
    const cabinValue = (ferryForm.cabin.value || "").trim();
    const cabinLower = cabinValue.toLowerCase();

    let cabinAnswer = "";
    if (cabinLower.startsWith("no cabin")) {
      cabinAnswer = "No, thank you. Just a seat, please.";
    } else if (cabinLower.includes("inside cabin")) {
      cabinAnswer = "Yes, please. I’d like an inside cabin, please.";
    } else if (cabinLower.includes("outside cabin")) {
      cabinAnswer = "Yes, please. I’d like an outside cabin with a window, please.";
    } else {
      // Fallback if you add more options later
      cabinAnswer = "Yes, please. I’d like " + cabinValue + ", please.";
    }

    const text =
      "Ticket clerk: Good morning. Where are you travelling to?\n" +
      name + ": Good morning. I’d like " + ticketType + " from " + route + ", please.\n" +
      "Ticket clerk: Are you travelling on foot, or with a vehicle?\n" +
      name + ": " + passengerAnswer + "\n" +
      "Ticket clerk: Would you like a cabin?\n" +
      name + ": " + cabinAnswer + "\n" +
      "Ticket clerk: What time are you travelling?\n" +
      name + ": I’m travelling " + time + ".\n" +
      "Ticket clerk: No problem. May I see your passport or ID card, please?\n" +
      name + ": Of course. Here you are.\n" +
      "Ticket clerk: Thank you. Boarding usually starts about 45 minutes before departure. Have a good journey!\n" +
      name + ": Thank you very much.\n\n" +
      "Narrator: " + name + " is ready to board the ferry. They are thinking about their trip to London and practising their English.";
...
/* ==============================
  ...
  if (meetForm && meetOutput) {
    meetForm.addEventListener("submit", (e) => {
      ...
      const companion = (meetForm.companion && meetForm.companion.value) ? meetForm.companion.value : "I’m travelling alone.";
      const reason = meetForm.reason.value || "I’m visiting London for the weekend.";

      const text =
        friendName + ": Hi, is this seat free?\n" +
        yourName + ": Yes, of course. Please go ahead.\n" +
        ...
        yourName + ": " + companion + "\n" +
        friendName + ": What brings you to London?\n" +
        yourName + ": " + reason + "\n" +
        "\nNarrator: " + yourName +
        " is practising small talk in English on the ferry. They are getting ready for their weekend in London.";
