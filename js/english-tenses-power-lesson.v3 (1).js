...
(() => {
  ...
  const IRREG = {
    ...
    travel:{past:'traveled', pp:'traveled'}
  };
  ...
  function isConsonant(ch){
    ...
    return /[a-z]/.test(c) && !VOWELS.has(c);
  }
  function isVowel(ch){
    if (!ch) return false;
  ...
  function isCVC(verb){
    if (!verb || verb.length < 3) return false;
  ...
  function pastForm(verb){
    ...
    if (v.endsWith('y') && verb.length>1 && isConsonant(v[v.length-2])) return verb.slice(0,-1) + 'ied';
    if (v.endsWith('c')) return verb + 'ked';
    if (isCVC(v)) return verb + verb[verb.length-1] + 'ed';
    return verb + 'ed';
  }
  ...
  function ingForm(verb){
    ...
    if (isCVC(v)) return verb + verb[verb.length-1] + 'ing';
    return verb + 'ing';
  }
  function thirdPersonS(verb){
    ...
    if (v === 'do') return 'does';
    if (v === 'go') return 'goes';
    if (v.endsWith('y') && verb.length>1 && isConsonant(v[v.length-2])) return verb.slice(0,-1)+'ies';
    if (/(s|sh|ch|x|z|o)$/.test(v)) return verb + 'es';
    return verb + 's';
  }
...
const TRANS_TARGETS = [
  ...
  function wireCoach(tense){
    ...
    function build(){
      const S = document.getElementById('coachSubj').value;
      const v = document.getElementById('coachVerb').value;
      const extraRaw = document.getElementById('coachExtra').value;
      ...
      switch (tense.id){
        case 'pres_simple':
          return `${S} ${isHeSheIt ? thirdPersonS(v) : v}${extra}.`;

        case 'pres_cont':
          return `${S} ${be} ${ingForm(v)}${extra}.`;

        case 'past_simple':
          return `${S} ${pastForm(v)}${extra}.`;

        case 'past_cont':
          return `${S} ${wasWere} ${ingForm(v)}${extra}.`;

        case 'pres_perf':
          return `${S} ${have} ${ppForm(v)}${extra}.`;

        case 'pres_perf_cont':
          return `${S} ${have} been ${ingForm(v)}${extra}.`;

        case 'past_perf':
          return `${S} had ${ppForm(v)}${extra}.`;

        case 'past_perf_cont':
          return `${S} had been ${ingForm(v)}${extra}.`;

        case 'future_will':
          return `${S} will ${v}${extra}.`;

        case 'future_going':
          return `${S} ${be} going to ${v}${extra}.`;

        case 'future_cont':
          return `${S} will be ${ingForm(v)}${extra}.`;

        case 'future_perf':
          return `${S} will have ${ppForm(v)}${extra}.`;

        case 'future_perf_cont':
          return `${S} will have been ${ingForm(v)}${extra}.`;
    ...
    function generate(){
      const s = build().replace(/\s+/g,' ').replace(' .','.');
      out.textContent = s;
      if (typeof setPill === 'function') setPill(msg,'Generated. Read it out loud!','good');
      if (typeof award === 'function') award(`coach_${tense.id}`, 3);
    }

    const btnGen = document.getElementById('coachGen');
    const btnSay = document.getElementById('coachSay');
    const btnCopy = document.getElementById('coachCopy');

    // overwrite handlers each render (tense changes re-render this whole block)
    btnGen.onclick = generate;
    btnSay.onclick = () => (typeof speak==='function' ? speak(out.textContent) : null);
    btnCopy.onclick = async () => {
      try{
        await navigator.clipboard.writeText((out.textContent||'').trim());
        if (typeof setPill === 'function') setPill(msg,'Copied ✔','good');
        if (typeof award === 'function') award(`coach_copy_${tense.id}`, 2);
      }catch{
        if (typeof setPill === 'function') setPill(msg,'Copy failed (browser).','warn');
      }
    };

    if (typeof bumpMax === 'function'){ bumpMax(3); bumpMax(2); }
    generate();
  }
  ...
  function initTransformer(){
    const baseSel = document.getElementById('transBase');
    const targetSel = document.getElementById('transTarget');
    const out = document.getElementById('transOut');
    const msg = document.getElementById('transMsg');

    if (!baseSel || !targetSel || !out) return;
    ...
    const targets = [
      {id:'pres_simple', label:'Present simple'},
      {id:'pres_cont', label:'Present continuous'},
      {id:'past_simple', label:'Past simple'},
      {id:'past_cont', label:'Past continuous'},
      {id:'pres_perf', label:'Present perfect'},
    ...
    if (!baseSel.value) baseSel.value = bases[0].id;
    if (!targetSel.value) targetSel.value = 'pres_perf';
    ...
    function build(){
      ...
      const have = isHeSheIt ? 'has' : 'have';
      const wasWere = (subj==='I'||isHeSheIt) ? 'was' : 'were';

      const v = b.verb;
      const obj = b.obj ? (' ' + b.obj) : '';

      switch (targetSel.value){
        case 'pres_simple': return `${subj} ${isHeSheIt ? thirdPersonS(v) : v}${obj}.`;
        case 'pres_cont': return `${subj} ${be} ${ingForm(v)}${obj}.`;
        case 'past_simple': return `${subj} ${pastForm(v)}${obj}.`;
        case 'past_cont': return `${subj} ${wasWere} ${ingForm(v)}${obj}.`;
    ...
    function render(givePoints){
      out.textContent = build().replace(/\s+/g,' ').replace(' .','.');
      if (typeof setPill === 'function') setPill(msg, 'Generated. Explain why the meaning changes.', 'good');
      if (givePoints && typeof award === 'function') award('trans_gen', 4);
    }

    // Buttons
    const btnGen = document.getElementById('transGenerate');
    const btnSpeak = document.getElementById('transSpeak');
    const btnCopy = document.getElementById('transCopy');
    const btnReset = document.getElementById('transReset');

    if (btnGen) btnGen.onclick = () => render(true);
    if (btnSpeak) btnSpeak.onclick = () => (typeof speak==='function' ? speak(out.textContent) : null);
    if (btnCopy) btnCopy.onclick = async () => {
      try{
        await navigator.clipboard.writeText((out.textContent||'').trim());
        if (typeof setPill === 'function') setPill(msg,'Copied ✔','good');
        if (typeof award === 'function') award('trans_copy', 2);
      }catch{
        if (typeof setPill === 'function') setPill(msg,'Copy failed.','warn');
      }
    };
    if (btnReset) btnReset.onclick = () => {
      if (typeof resetTask === 'function'){ resetTask('trans_gen',4); resetTask('trans_copy',2); }
      render(false);
    };

    // Auto-update on dropdown change (no points)
    baseSel.onchange = () => render(false);
    targetSel.onchange = () => render(false);

    if (typeof bumpMax === 'function'){ bumpMax(4); bumpMax(2); }
    render(false);
  }
