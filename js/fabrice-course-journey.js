...
(function(){
  const sessions = Array.isArray(window.FABRICE_SESSIONS) ? window.FABRICE_SESSIONS.slice() : [];
  const categoryMeta = window.FABRICE_CATEGORY_META || {};
  const story = Array.isArray(window.FABRICE_STORY) ? window.FABRICE_STORY.slice() : [];
  ...
  function renderResources(){
    ...
    if(resultsSummary){
      ...
      resultsSummary.textContent = `${filtered.length} session${filtered.length === 1 ? '' : 's'} shown • ${countLinks(filtered)} lesson link${countLinks(filtered) === 1 ? '' : 's'} • ${activeLabel}`;
    }
  ...
  copyLibraryNoteBtn?.addEventListener('click', function(){
    const text = 'This archive brings together the full story of Fabrice’s English journey: dates, lesson themes, key content, and direct links to every resource used throughout the training.';
    copyText(text).then(function(){ flashButton(copyLibraryNoteBtn, 'Copied ✓'); });
  });

  copyNoteBtn?.addEventListener('click', function(){
    const text = 'This page is here to remind you that your journey was real, rich, and deeply meaningful. You worked with seriousness, heart, and courage. Your progress deserves to be seen. Keep this page as proof of everything you built.';
