...
window.__INTRO_DATA__ = {
      ...
      "slides":[{"n":1,"lines":["🏨","▶ Start Lesson","– GETTING TO KNOW EACH OTHER–","🧳","✈️","🗺️","INTRODUCTION","– ENGLISH LESSON –","📞","LEARNER: FRANKIE DUMARQUEZ | DURATION: 60 MIN | FOCUS: INTRODUCTIONS, GREETINGS, COMMUNICATION NEEDS"]},
              {"n":2,"lines":["YANIS","“Hello Tisha! Nice to meet you!”","💬","How are you today?","😊😴😄 to say how you feel today.","👶 Do you have children?","🏠 How many people are in your family?","🐶🐱 Do you have any pets?","🗓️ What do you do on weekends?","🎨🎵🏃‍♀️ What do you do in your free time?"]},
...
// flip handler (ignore icon button)
  ...
  ----------------------------*/
    /* ---------------------------
     Verification checklist (beginning of program)
  ----------------------------*/
    /* ---------------------------
     Verification checklist (beginning of program)
  ----------------------------*/
  function initVerifyChecklist() {
    var wrap = $("verifyTableWrap");
    var table = $("verifyTable");
    if (!wrap || !table) return;

    var STORAGE_KEY = "speakeasytisha.intro.verify.v2";
    var state;
    ...
    // Bind each row
    var rows = $all("#verifyTable tbody tr[data-id]");
    rows.forEach(function (row) {
      var id = row.getAttribute("data-id");
      var cb = row.querySelector(".verifyCheck");
      var sel = row.querySelector(".coverSelect");
      if (!id) return;

      if (cb) cb.checked = !!state.checks[id];
      if (sel) sel.value = state.covers[id] || "";
    ...
    function exportCSV() {
      var lines = [];
      lines.push(["Statut", "Compétence / besoin", "Validé", "À couvrir ?"].join(";"));

      // groups + rows in order
      var bodyRows = $all("#verifyTable tbody tr");
      bodyRows.forEach(function (tr) {
        if (tr.classList.contains("groupRow")) {
          var g = tr.textContent.trim();
          lines.push("");
          lines.push([g, "", "", ""].join(";"));
          return;
        }
        ...
        var full = label + (text ? " — " + text : "");
        var validated = state.checks[id] ? "Oui" : "Non";
        var cover = coverLabel(state.covers[id] || "");

        var row = [status, full, validated, cover].map(function (s) {
          var v = String(s || "");
      ...
      lines.push("");
      lines.push(["Date de démarrage", (sd ? (sd.value || "") : ""), "", ""].join(";"));

      var csv = lines.join("\n");
      var blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
      var a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = "speakeasytisha-verification.csv";
    ...
    var exp = $("btnVerifyExport");
    var res = $("btnVerifyReset");
    if (exp) exp.addEventListener("click", exportCSV);
    if (res) res.addEventListener("click", resetAll);
  }
